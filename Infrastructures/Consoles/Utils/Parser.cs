...
public static class Parser
{
    ...
    /// <returns></returns>
    public static string FillFormatTostring(this List<ColumnShowFormat> format, string[] objs, string delimiterStr,
        int columnBlankNum)
    {
        ...
        format.ForEach(f =>
        {
            ...
            int alignmentStrLength =
                Math.Max(f.StrLength - objs[f.Index].ObjToString().GetChineseText().Length, 0); //对其填充空格数量
    ...
    /// <returns></returns>
    public static string GetTitleStr(this List<int> columnWides, string titleStr, int columnBlankNum,
        string delimiterStr)
    {
    ...
    {
        string divider = "";
        columnWides.ForEach(i => { divider += $"{angleStr}{"".PadRight(i + columnBlankNum * 2, '-')}"; });
        divider += angleStr;
        return divider;
    }
    ...
    /// <param name="columnBlankNum">列两端留白数</param>
    /// <returns></returns>
    public static string GetTopAndDown(this List<int> columnWides, string angleStr, int columnBlankNum)
    {
        string top_DownDividerdivider = "";
        columnWides.ForEach(i =>
        {
            if (string.IsNullOrWhiteSpace(top_DownDividerdivider))
                top_DownDividerdivider += $"{angleStr}{"".PadRight(i + columnBlankNum * 2, '-')}";
