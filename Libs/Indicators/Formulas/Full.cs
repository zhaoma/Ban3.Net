        ...
        public void Calculate(List<Inputs.Price> prices)
        {
            ...
            for (var i = 0; i < prices.Count; i++)
            {
                ...
                if (i >= bias.N - 1)
                {
                    ...
                    if (biasMA != 0)
                    {
                        Result.EndPoints[i].Bias.RefBIAS = Math.Round((prices[i].CurrentClose.Value - biasMA) / biasMA * 100M, 3);
                    }
                }
                ...
                try
                {
                    if (i > 0)
                    {
                        ...
                        middles[i][3] = Result.EndPoints[i].Dmi.RefHD.Value > 0 && Result.EndPoints[i].Dmi.RefHD.Value > Result.EndPoints[i].Dmi.RefLD.Value ? Result.EndPoints[i].Dmi.RefHD.Value : 0M;
                        middles[i][4] = Result.EndPoints[i].Dmi.RefLD.Value > 0 && Result.EndPoints[i].Dmi.RefLD.Value > Result.EndPoints[i].Dmi.RefHD.Value ? Result.EndPoints[i].Dmi.RefLD.Value : 0M;
                    ...
                    if (i >= dmi.M - 1)
                    {
                        ...
                        for (int r = i; r > i - dmi.M; r--)
                        {
                            adx += Result.EndPoints[r].Dmi.RefPDI + Result.EndPoints[r].Dmi.RefMDI == 0M
                                           ? 0M
                                           : Math.Abs(Result.EndPoints[r].Dmi.RefMDI.ToDecimal() - Result.EndPoints[r].Dmi.RefPDI.ToDecimal()) /
                                             (Result.EndPoints[r].Dmi.RefPDI.ToDecimal() + Result.EndPoints[r].Dmi.RefMDI.ToDecimal()) * 100M;
                        }
                    ...
                    if (i >= 2 * dmi.M - 1)
                    {
                        Result.EndPoints[i].Dmi.RefADXR = (Result.EndPoints[i].Dmi.RefADX.ToDecimal() + Result.EndPoints[i - dmi.M].Dmi.RefADX.ToDecimal()) / 2;
                    }
                ...
                if (i >= ene.N - 1)
                {
                    ...
                    Result.EndPoints[i].Ene.RefENE = Math.Round((Result.EndPoints[i].Ene.RefUPPER.Value + Result.EndPoints[i].Ene.RefLOWER.Value) / 2, 2);
                }
                ...
                try
                {
                    ...
                    {
                        Result.EndPoints[i].Kd.RefDailyPSV = Math.Round((prices[i].CurrentClose.Value - Result.EndPoints[i].Kd.RefLLV.Value) * 100M / (Result.EndPoints[i].Kd.RefHHV.Value - Result.EndPoints[i].Kd.RefLLV.Value), 3);
                    }
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        Result.EndPoints[i].Macd.RefDIF = Math.Round(Result.EndPoints[i].Macd.RefEMAShort - Result.EndPoints[i].Macd.RefEMALong, 3);
                        Result.EndPoints[i].Macd.RefDEA = Math.Round(deaYest * (macd.MID - 1) / (macd.MID + 1) + Result.EndPoints[i].Macd.RefDIF * 2 / (macd.MID + 1), 3);
                        Result.EndPoints[i].Macd.RefMACD = Math.Round(2 * (Result.EndPoints[i].Macd.RefDIF - Result.EndPoints[i].Macd.RefDEA), 3);
                    }
